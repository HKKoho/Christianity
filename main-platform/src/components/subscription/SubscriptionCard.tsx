...
export function SubscriptionCard({ plan, onSelect }: SubscriptionCardProps) {
  ...
  return (
    <div
      className={`relative bg-white rounded-2xl p-6 flex flex-col transition-all duration-300 hover:shadow-2xl hover:scale-105 ${cardClasses}`}
    >
      {plan.isFeatured && (
      ...
      <div className="mt-4">
        <span className="text-3xl font-extrabold text-slate-900">{plan.priceText}</span>
        {plan.trialPeriodDays && (
          <p className="text-xs text-slate-500">{t('subscription.no_credit_card')}</p>
        )}
      </div>
      <ul className="mt-6 space-y-3 flex-grow">
        {plan.features.map((feature, index) => (
          <li key={index} className="flex items-start">
            <CheckIcon className="h-5 w-5 text-purple-600 flex-shrink-0 mr-2" />
